 ...
 */
public class SCompoundCommand extends SCommand {

    /** The sequence of commands. */
    private final List<SCommand> sequence;

    public SCompoundCommand() {
        this(false);
    }
     ...
     */
    public SCompoundCommand(final boolean executed) {
        super(executed);
     ...
     */
    public void add(final SCommand command) throws IllegalStateException {
        if (command == null) {
            // ignore
            return;
        }
     ...
     */
    public void addAll(final SCompoundCommand compound)
    ...
    public void execute() {
        ...
        for (final SCommand command : sequence) {
            command.execute();
        }
    }
    ...
    public void revert() {
        ...
        for (int i = sequence.size() - 1; 0 <= i; --i) {
            final SCommand command = sequence.get(i);
            command.revert();
        }
    }
    ...
    public Collection<SCell> getCells() {
        final Collection<SCell> result = super.getCells();
        for (final SCommand command : sequence) {
